...
```bash
# from repo root
uv run python -m vera.train --model cnn \
...
git add web apps/api.py src/vera/inference.py pyproject.toml \
...
4. Print a preview URL (e.g. `https://vera-xxxx.vercel.app`)
...
(`https://vera.vercel.app` or your custom domain).
